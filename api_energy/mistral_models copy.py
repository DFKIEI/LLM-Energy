...
def generate(model, tokenizer, prompt: str, max_input_tokens: int, max_new_tokens: int,
    ...
    actual_output_length = len(gen_ids)
    text = tokenizer.decode(gen_ids, skip_special_tokens=True)
    return text.strip(), actual_input_length, actual_output_length
...
def main():
    ...
    print(f"Total samples in CSV: {len(all_queries_data)}")
    ...
    with open(args.out_csv, "w", newline="", encoding="utf-8") as fout:
        ...
        for i in range(0, len(queries_data), batch_size):
            ...
            # Write batch results
            for j, (query, (_, _, qtype, otype), (response, in_tokens, out_tokens)) in enumerate(zip(batch_queries, batch_params, batch_responses)):
                writer.writerow({
                    "query": query,
                    "query_type": qtype,
                    "output_type": otype,
                ...
                if otype in output_type_counts:
                    output_type_counts[otype] += 1
    ...
    print(f"Avg Input Tokens/Query: {total_input_tokens/total_queries:.1f}")
    print(f"Avg Output Tokens/Query: {total_output_tokens/total_queries:.1f}")
    print(f"\nQuery Type Distribution:")
    print(f"  Short: {query_type_counts['short']} ({100*query_type_counts['short']/total_queries:.1f}%)")
    print(f"  Long: {query_type_counts['long']} ({100*query_type_counts['long']/total_queries:.1f}%)")
    print(f"\nOutput Type Distribution:")
    print(f"  Short: {output_type_counts['short']} ({100*output_type_counts['short']/total_queries:.1f}%)")
    print(f"  Long: {output_type_counts['long']} ({100*output_type_counts['long']/total_queries:.1f}%)")
    print("="*70)
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"Avg Input Tokens/Query: {total_input_tokens/total_queries:.1f}\n")
        f.write(f"Avg Output Tokens/Query: {total_output_tokens/total_queries:.1f}\n")
        f.write(f"\nQuery Type Distribution:\n")
        f.write(f"  Short: {query_type_counts['short']} ({100*query_type_counts['short']/total_queries:.1f}%)\n")
        f.write(f"  Long: {query_type_counts['long']} ({100*query_type_counts['long']/total_queries:.1f}%)\n")
        f.write(f"\nOutput Type Distribution:\n")
        f.write(f"  Short: {output_type_counts['short']} ({100*output_type_counts['short']/total_queries:.1f}%)\n")
        f.write(f"  Long: {output_type_counts['long']} ({100*output_type_counts['long']/total_queries:.1f}%)\n")
...
def generate_batch(model, tokenizer, prompts, params, temperature, top_p, top_k, repetition_penalty):
    ...
    for prompt, (max_in, max_out, _, _) in zip(prompts, params):
        ...
        response, input_len, output_len = generate(
            model, tokenizer, prompt, max_in, max_out,
            temperature, top_p, top_k, repetition_penalty
        )
        responses.append((response, input_len, output_len))
